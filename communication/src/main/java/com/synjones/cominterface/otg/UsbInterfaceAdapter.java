...
public class UsbInterfaceAdapter implements DataTransInterface {
       ...
       UsbDeviceConnection usbDeviceConnection = usbManager.openDevice(usbDevice);
        if (usbDeviceConnection != null) {
          ...
          //  Log.i(TAG, "getInterfaceCount()=>" + device.getInterfaceCount());
            ...
            if(usbInterface.getEndpointCount()==2){//新模块
    ...
    public void sendData(byte[]   data, int datalen) {
        ...
        if(ueout32!=null){
            ...
            usbDeviceConnection.bulkTransfer(ueout32, sendBuf, sendBuf.length, writeTimeOut);
            offset=7;
            datalen-=7;
        }
        ...
        while(datalen>0){
            ...
            sendBuf=new byte[Math.min(64,datalen)];
            System.arraycopy(data,offset,sendBuf,0,sendBuf.length);
    ...
    public int recvData(byte[] recvbuf, int offset) {
        ...
        int readlen32=0;
        if(offset<7 && uein32!=null){
            readlen32 = usbDeviceConnection.bulkTransfer(uein32, buffer, 32, readTimeOut);
            if(readlen32<=0) return readlen32;
            System.arraycopy(buffer,0,recvbuf,offset,readlen32);
            offset+=readlen32;
            //Log.e("USB", util.bytesToHexString(buffer,readlen32));
        }

        readlen = usbDeviceConnection.bulkTransfer(uein, buffer, 64, readTimeOut);
        if(readlen<=0) return readlen;
       // Log.e("USB", util.bytesToHexString(buffer,readlen));
