...
public class M1CardReader implements M1ReaderInterface {
	...
	* @return 4字节卡序列号+ 1字节（保留）+1字节卡类型+2字节ATQ字节
	...
	public M1CardInfo findM1Card(){
		CmdResponse resp= doM1ReaderCmd(0, null, 500);
		if(resp==null) return null;
		return new M1CardInfo(resp.getData());
	}
	...
	* @return true 成功
	...
	public boolean verifyKey(KeyType type, int blockNo, byte key[]){
		...
		CmdResponse resp= doM1ReaderCmd(0x1, data, 200);
		if(resp!=null)
			return true;
		/*else{
			pend();
	...
	* @return 16字节数据
	...
	public byte[] readBlock(int blockNo){
		...
		CmdResponse resp= doM1ReaderCmd(0x2, data, 200);
		//if(resp==null) 
	...
	* @return true 成功
	...
	public boolean writeBlock(int blockNo,byte writeData[]){
		...
		CmdResponse resp= doM1ReaderCmd(0x3, data, 200);
		if(resp!=null)
			return true;
		//else
	...
	* @return ture 成功
	 */
	public boolean pend(){
		CmdResponse resp= doM1ReaderCmd(0x6, null, 200);
		if(resp!=null)
			return true;
		return false;
	}
