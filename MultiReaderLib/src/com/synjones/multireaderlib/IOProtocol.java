...
public class IOProtocol {
	...
	* @return
	...
	public byte[] rawSendRecv(byte data[]){
        send(data);
        util.mSleep(200);
        byte[] buf= new byte[100];
        int len=dti.recvData(buf,0);
        if(len>0){
    ...
    public byte[] sendAndRecvZkData(byte data[]){
		send(data);
		long tickStart = System.currentTimeMillis();
		int totalsize = 6;
		int readSize = 0;
		int realLen = 0;
		byte[] mTempBuffer = new byte[4096];
		byte[] mOutBuffer = new byte[4096];
		int requestSize = mTempBuffer.length;

		ZkReaderRetData zkReaderRetData=new ZkReaderRetData();
		zkReaderRetData.free();
		while(Math.abs(System.currentTimeMillis() - tickStart) < (long)500) {

			int size = dti.recvData(mOutBuffer, readSize);
			if (size > 0) {
				readSize += size;
			}

			if (realLen == 0 && readSize >= totalsize) {
				realLen = zkReaderRetData.PreGetLen(mOutBuffer);
				totalsize = realLen;
			}
		...
		if(!zkReaderRetData.UnPacket(mOutBuffer)){
		...
		int Len = zkReaderRetData.getDataLen();
		byte[] dataOut = new byte[Len - 1];
		System.arraycopy(zkReaderRetData.getData(), 0, dataOut, 0, Len - 1);
		return dataOut;
	}
	...
	private void send(byte data[]){
		dti.clear();
		//Log.e("IO lib", "send: "+util.bytesToHexStringNo0x(data));
	...
	public byte[] recv(ProtocolType Type,long timeout){
		...
		long recvTimeout=0;
		while(System.currentTimeMillis()-cur_time<timeout && !Thread.interrupted()){
